...
import { ORDER, buildGeometry, drawGame, hitToken, lerpPos } from './ludoPentCore.js';

const SPEED = 0.011;   // cells per ms (cell-by-cell token movement)

/** LudoPentCanvas — 5-player pentagonal board with cell-by-cell token movement. */
export default function LudoPentCanvas({ players = [], activeColor, movable = new Set(), onToken }) {
  ...
  return (
    <div ref={wrapRef} className="ludo-canvas-wrap" style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
      <canvas ref={canvasRef} onClick={handleClick} role="img" aria-label="Pentagonal Ludo board"
